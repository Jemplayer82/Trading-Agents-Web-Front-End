...
def create_aggressive_debator(llm):
    def aggressive_node(state) -> dict:
        ...
        if count < 3:
            ...
            source_block = f"Research Manager's Investment Plan: {investment_plan}"
            no_responses_note = ""

        if count == 0:
            no_responses_note = (
                " If there are no responses from the other viewpoints yet, "
...
{source_block}
Here is the current conversation history: {history}.{no_responses_note}
