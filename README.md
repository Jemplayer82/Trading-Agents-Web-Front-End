...
- Daily options paper trader (movers pre-screen top 250 → quick scan top 100 + SPY → deep-dive top 25 → long calls/puts with hard risk guardrails, real cash/realized-P&L ledger)
...
- **07:30 ET** — momentum/volume pre-screen ranks the top **250** S&P movers; the top **100 + SPY** (101 total) get the quick LLM scan; the top **25 directional names** (BUY *and* SELL — big losers become put candidates) get the full multi-agent deep dive
